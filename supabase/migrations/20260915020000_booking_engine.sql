-- Availability and customer appointment mutations.
...
begin
  if p_treatment_id is null or p_date is null then
    return;
  end if;
  ...
  if treatment_duration is null then
    return;
  end if;
  ...
  loop
    ...
    while slot_minutes + treatment_duration <= period_end_minutes loop
      slot_start_local := p_date::timestamp + make_interval(mins => slot_minutes);
      slot_end_local := p_date::timestamp
        + make_interval(mins => slot_minutes + treatment_duration);
      slot_start := slot_start_local at time zone business_timezone;
      slot_end := slot_end_local at time zone business_timezone;
...
begin
  ...
  if p_start_time is null then
    raise exception using errcode = '22023', message = 'An appointment time is required';
  end if;
  ...
  select available.starts_at, available.ends_at
    into selected_slot
  from public.get_available_slots(p_treatment_id, p_date) available
...
begin
  select coalesce(cancellation_notice_hours, 24)
    into notice_hours
  from public.business_settings settings
  where settings.id = true;
  ...
  if not found or appointment_record.status <> 'confirmed' then
    raise exception using errcode = '42501', message = 'Appointment cannot be cancelled';
  end if;

  if appointment_record.starts_at <= now() + make_interval(hours => notice_hours) then
    raise exception using errcode = '42501',
      message = format('Appointments can only be cancelled at least %s hours in advance', notice_hours);
  end if;
...
revoke execute on function public.get_available_slots(uuid, date, uuid)
  from public, anon, authenticated;
grant execute on function public.get_available_slots(uuid, date, uuid)
  to anon, authenticated, service_role;

revoke execute on function public.book_appointment(uuid, date, time, text, text, text)
  from public, anon, authenticated;
grant execute on function public.book_appointment(uuid, date, time, text, text, text)
  to service_role;

revoke execute on function public.cancel_customer_appointment(uuid, text)
  from public, anon;
grant execute on function public.cancel_customer_appointment(uuid, text)
  to authenticated;

revoke execute on function public.reschedule_customer_appointment(uuid, date, time)
  from public, anon;
grant execute on function public.reschedule_customer_appointment(uuid, date, time)
  to authenticated;
